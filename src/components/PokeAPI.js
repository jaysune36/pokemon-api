...
const POKELIST_API_HTML = `https://pokeapi.co/api/v2/`;

class PokeListAPI {
  get = async(strUrl, numSet, arr, changeBooState) => {
    try {
      changeBooState(true);
      // console.log(`${POKELIST_API_HTML}${strUrl}${numSet}`)
      for(let i=0; i < numSet; i++) {
        const resp1 = await fetch(`${POKELIST_API_HTML}${strUrl}${i + 1}/`);
...
class PokeListAPIIndividual {
  get = async(url, changeBooStateInd) => {
    try {
      const resp = await fetch(`${POKELIST_API_HTML}${url}`)
